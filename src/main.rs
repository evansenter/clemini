...
- `read_file` - Read specific files you know exist. For large files, use `offset` and `limit` to read targeted sections. Check the `truncated` field to see if more content remains.
