...
pub struct EventBusNotifyTool {
    events_tx: Option<mpsc::Sender<AgentEvent>>,
}

impl EventBusNotifyTool {
    pub fn new(events_tx: Option<mpsc::Sender<AgentEvent>>) -> Self {
        Self { events_tx }
    }
}

impl ToolEmitter for EventBusNotifyTool {
    fn events_tx(&self) -> &Option<mpsc::Sender<AgentEvent>> {
        &self.events_tx
    }
}
...
#[async_trait]
impl CallableFunction for EventBusNotifyTool {
    fn declaration(&self) -> FunctionDeclaration {
        FunctionDeclaration::new(
            "event_bus_notify".to_string(),
            "Send a system notification (desktop notification).".to_string(),
            FunctionParameters::new(
                "object".to_string(),
                json!({
                    "title": {
                    ...
                    "sound": {
                        "type": "boolean",
                        "description": "Play sound (default: false)"
                    }
                }),
    ...
    async fn call(&self, args: Value) -> Result<Value, FunctionError> {
        let title = args
            .get("title")
            .and_then(|v| v.as_str())
            .ok_or_else(|| FunctionError::ArgumentMismatch("title is required".into()))?;

        let message = args
            .get("message")
            .and_then(|v| v.as_str())
            .ok_or_else(|| FunctionError::ArgumentMismatch("message is required".into()))?;

        let _sound = args.get("sound").and_then(|v| v.as_bool()).unwrap_or(false);
        ...
        #[cfg(target_os = "linux")]
        {
            ...
            match status {
                Ok(s) if s.success() => {
                    self.emit(&format!("Notification sent: {}", title));
                    Ok(json!({ "success": true }))
                }
                Ok(_) => Ok(json!({
                    "success": false,
                    "error": "notify-send failed"
                })),
                Err(e) => Ok(json!({
                    "success": false,
                    "error": format!("notify-send not available: {}", e)
                })),
            }
        }
        ...
        #[cfg(target_os = "macos")]
        {
            ...
            match status {
                ...
                Err(e) => Ok(json!({
                    "success": false,
                    "error": format!("osascript not available: {}", e)
                })),
            }
        }
...
mod tests {
    ...
    #[tokio::test]
    async fn test_notify_requires_title_and_message() {
        let tool = EventBusNotifyTool::new(None);

        let result = tool.call(json!({ "message": "test" })).await;
        assert!(result.is_err());

        let result = tool.call(json!({ "title": "test" })).await;
        assert!(result.is_err());
    }
}
